...
(define proc-x5 (run "let makemult = proc (maker)
                                      proc (x)
                                        if zero?(x)
                                        then 0
...
(define proc-x6 (run "let times = proc(maker)
                                   proc(x)
                                    proc(y)
                                     if zero?(y)
                                     then 0
                                     else -((((maker maker) x) -(y,1)), -(0,x))
                  in let facMaker = proc(maker)
                                    proc(x)
                                    if zero?(x)
                                    then 1
                                    else (((times times) ((maker maker) -(x,1))) x)
                  in let factorial = proc(x) ((facMaker facMaker) x)
                       in (factorial 5)
"   
))

(define proc-x7 (run "let func = proc(maker)
                                 proc(x)
                                  if zero?(x)
                                  then zero?(x)            ;even return #t
                                  else
                                   if zero?(-(x,1))       ; x == 1 ?
                                   then zero?(x)
                                   else
                                     ((maker maker) -(x,2))
                     in let even = proc(x) ((func func) x)
                       in (even 4)
 "
)
)

(define proc-x8 (run "let func = proc(maker)
                                 proc(x)
                                  if zero?(x)
                                  then zero?(-(x,1))            ;even return #f
                                  else
                                   if zero?(-(x,1))       ; x == 1 ?
                                   then zero?(-(x,1))
                                   else
                                     ((maker maker) -(x,2))
                     in let odd = proc(x) ((func func) x)
                       in (odd 3)
 ")
)

; exercise3.25
(define proc-x9 (run " let makerec = proc (f)
                                     let d = proc (x)
                                             proc(z) ((f (x x)) z)
                                     in proc (n) ((f (d d)) n)
...
; (makerec maketimes4) -> proc(n) ((maketimes4 (d d)) n)        --> n = 3    -> ((maketimes4 (d d )) 3)
; (d d) -> proc(z) ((maketimes4 (d d)) z) -> (d d)作为f 代入maketimes4
; (f -(x,1)) -> (maketimes4 (d d) 2) 当x = 3 时, 然后接续代入 (maketimes4 (d d) 1) 直到0为终止。 
;
;
