...
(define grammar-a1
 '((program (expression) a-program)
   (proc_statement (identifier "(" (separated-list identifier ",") ")" "=" expression) procstatement)
   ...
   ; 3.31 letrec
   (expression ("letrec" (arbno proc_statement) "in" expression) letrec-exp)
  )
)
...
(define extend-env-rec
 (lambda (proc-name bound-var proc-body saved-env)
  (let ([val (proc-val (procedure bound-var proc-body saved-env #f))])
     (extend-env proc-name val saved-env)
  )
 )
)
...
(define-datatype expression expression?
 ...
 ; add letrec
 (letrec-exp (args (list-of proc_statement?)) (letrec-body expression?))
)

(define-datatype proc_statement proc_statement?
  (procstatement (p-name symbol?) (bound-var (list-of symbol?)) (p-body expression?))  
)
...
(define value-of
 (lambda (exp env)
   (cases expression exp
    ...
    ; add letrec-exp  multiple expression
    (letrec-exp (args letrec-body)
       (value-of letrec-body (extend-env-with-procstatement args env))
    )
  )
 ) 
)

(define extend-env-with-procstatement
 (lambda(list env)
   (if (null? list)
        env
        (let ([procVal (car list)])     
         (cases proc_statement procVal
           (procstatement (p-name bound-var p-body) (extend-env-with-procstatement (cdr list) (extend-env-rec p-name bound-var p-body env)) )
           (else (display "shold not be this type"))
         )
        )
   )  
 ) 
