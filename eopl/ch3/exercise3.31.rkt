...
(define letrec-x (run "letrec double(x,y) = if zero?(x) then 0 else -((double -(x,1) y),y)
                                  in (double 6 -4)")
)

; exercise3.30 What is the purpose of the call to proc-val on the next-to-last line of apply-env?
; suppose the initial env is `()  = a , we add {double (doule-proc-val with a env)} into a then we 
; call the new env is b .  
; when we execute the function "double" with a env , we come across a question : "what's the definition of double?"
; beacuse a env is empty , we can't find any definition about "double".
; so we call proc-val to change the env of "double" from a to b .
;
