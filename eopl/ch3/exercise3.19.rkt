...
(define grammar-a1
 '((program (expression) a-program)
   ...
   (expression ("letproc" identifier "=" "function" "("  identifier ")" expression "in" expression) proc-exp)
  )
)
...
(define-datatype expression expression?
 ...
 (proc-exp (name symbol?) (var symbol?) (body expression?) (call-exp expression?))
...
(define value-of
 (lambda (exp env)
   (cases expression exp
    ...
    (proc-exp (name var body call)
      (
        let ([proc (proc-val (procedure var body env))])
        (value-of call (extend-env name proc env))
      )
    )
    (call-exp (rator rand)
...
(define letproc-x (run "letproc f = function(x) -(x,11) in (f (f 77))" ))
