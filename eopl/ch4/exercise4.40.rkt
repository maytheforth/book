...
(define expval-or-thunk?
 (lambda (x)
  (if (or (expval? x) (thunk? x))
    #t
    #f
  )
 )
...
(define parse-one-arg-lazy
  (lambda (arg env store)
     (cases expression arg
      (const-exp (num)
        (cases answer (value-of arg env store)
         (an-answer (val new-store) (cons (list val) new-store))
        )
      )
     (proc-exp (vars body)
        (cases answer (value-of arg env store)
   ...
   (else
     (cons (list (a-thunk arg env)) store)
   )
  )
 )
)
...
(define parse-args-lazy
 (lambda (args env store)
  (cond
    [(null? args) (cons empty store)]
    [(= (length args) 1)
      (parse-one-arg-lazy (car args) env store)
    ]
    [else
      (let* ([left-rest (parse-one-arg-lazy (car args) env store)]
             [rest (parse-args-lazy (cdr args) env (cdr left-rest))]
             [rest-val (car rest)]
             [rest-store (cdr rest)]
            )
         (cons (append (car left-rest) rest-val) rest-store)
      )
    ]
  )
 )
...
(define apply-var-exp-lazy
 (lambda (var env store)
  (let ([ret (apply-store store (apply-env env var))])
    (if (expval? ret)
       (an-answer ret store)
       (value-of-thunk ret store)
    )
  )
 )
)
...
(define value-of
 (lambda (exp env store)
  (cases expression exp
    ...
    (var-exp (var) (apply-var-exp-lazy var env store))
    (diff-exp (exp1 exp2)
     ...
     (cases answer (value-of rator env store)
       (an-answer (val store1)
         (let* ([proc (expval->proc val)]
                [rest (parse-args-lazy rand env store1)]
                [rest-store (cdr rest)]
                [rest-ret (car rest)]
               )
  ...
  (ref-exp (var)
    (an-answer (ref-val (apply-env env var)) store)
  )
 )
)
)
...
(define value-of-thunk
 (lambda (th store)
  (cases thunk th
   (a-thunk (expression env)
     (value-of expression env store) 
   )
  (else (eopl:error "error ,not a thunk"))
  )
 )
)
...
; lazy evaluation
(define test3 (run "
         letrec infinite-loop(x) = proc(x) (infinite-loop -(x,1))
         in let f = proc (z) 11
